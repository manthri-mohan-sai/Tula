...
final class Expense {
    ...
    /// Populated by the parser for multi-item entries; empty for simple ones.
    /// Cascade-deletes with the expense.
    ...
    /// "EMI 2/3" when this expense is one installment of an EMI plan, else nil.
    /// Index = this installment's position by date among the plan's expenses.
    var emiLabel: String? {
        guard let rule = recurringRule, rule.isEMI, rule.installmentTotal > 0 else { return nil }
        let index = rule.generatedExpenses.filter { $0.date <= date }.count
        return "EMI \(max(index, 1))/\(rule.installmentTotal)"
    }
...
/// A single purchased item within an expense. Kept lightweight — a name and an
/// optional per-item price (voice/quick-log items usually have no price; only
/// receipts do). Belongs to exactly one `Expense`.
@Model
final class LineItem {
    var id: UUID = UUID()
    var name: String = ""
    /// Per-item price when known (receipts); nil for free-text items.
    var price: Double? = nil
    var createdAt: Date = Date()

    /// Owning expense (inverse of `Expense.items`).
    var expense: Expense?

    init(name: String, price: Double? = nil) {
        self.name = name
        self.price = price
    }
}
...
final class RecurringRule {
    ...
    // MARK: - EMI / installment fields

    /// When true, this rule is a finite EMI/installment plan (created by
    /// EMIPlanner), not an open-ended recurring expense. Drives the "EMI X/N"
    /// badge and outstanding-balance display.
    var isEMI: Bool = false

    /// Total number of installments in the EMI plan. 0 for non-EMI rules.
    var installmentTotal: Int = 0

    /// Installments already generated / paid. Derived from linked expenses.
    var installmentsPaid: Int { generatedExpenses.count }

    /// Amount still owed across the remaining installments. 0 when complete.
    var outstandingAmount: Double {
        guard isEMI, installmentTotal > 0 else { return 0 }
        let remaining = max(installmentTotal - installmentsPaid, 0)
        return Double(remaining) * amount
    }
