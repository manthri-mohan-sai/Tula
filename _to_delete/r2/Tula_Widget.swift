...
/// Widget-target mirror of the app's `formatDeltaPercent` (Insights.swift).
/// Percentages up to 100% read fine as-is; above 100% switches to a
/// one-decimal multiplier ("2.5x" instead of "250%").
private func formatDeltaLabel(_ change: Double) -> String {
    let absChange = abs(change)
    let percent = Int((absChange * 100).rounded())
    if percent > 100 {
        let multiplier = (absChange * 10).rounded() / 10 // one decimal
        if multiplier == multiplier.rounded() {
            return "\(Int(multiplier))x" // drop trailing .0 → "3x"
        }
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                if let pct = dayChangePercent {
                    HStack(spacing: 2) {
                        ...
                        Text("\(formatDeltaLabel(pct)) vs same day last month")
                        ...
                        if hasBudget {
                            ...
                            VStack(alignment: .leading, spacing: 5) {
                                HStack(spacing: 4) {
                                    ...
                                    Text("\(formatDeltaLabel(pct)) vs \(lastMonthName)")
                                        .font(.system(size: 11, weight: .semibold))
                                        .lineLimit(1)
                                        .minimumScaleFactor(0.7)
                                }
